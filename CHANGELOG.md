...
All notable changes to Linnaeus will be documented in this file.
...
### Added
- `linnaeus` Python package (`src/linnaeus/`) with CLI, configuration, data
...
- Multi-taxonomy support: builtin `linnaeus` (20 top-level categories),
  `asdb` (Stanford ASdb, 17), and `isic` (ISIC Rev.4, 20) taxonomies packaged
  under `linnaeus/resources/taxonomies/`; `--taxonomy` / `--taxonomy-file`
...
- Editable LLM prompt templates in `linnaeus/resources/prompts.yaml`
...
- Supported fine-tuning workflow (`linnaeus model fine-tune`,
  `linnaeus model prepare-data`) built on the validated end-to-end path, with
...
### Changed
- Canonicalized the linnaeus taxonomy definitions to the 20 canonical
...
### Removed
- `linnaeus model fine-tune-async` and the non-functional
  `LLMTrainingPipeline` / `LLMDataPreparation` classes (replaced by
  `linnaeus.models.llm.finetuning`)
